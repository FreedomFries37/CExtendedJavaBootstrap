...
import radin.midanalysis.typeanalysis.errors.IdentifierDoesNotExistError;
...
public class VariableTypeTracker {
    ...
    private static HashMap<AbstractCXClassType, VariableTypeTracker> classTrackers;
    ...
    private VariableTypeTracker(VariableTypeTracker old, AbstractCXClassType parentType) {
        this(old);
    ...
    public VariableTypeTracker createInnerTypeTracker(AbstractCXClassType owner) {
    ...
    public VariableTypeTracker createInnerTypeTrackerLoad(AbstractCXClassType owner) {
    ...
    public boolean isVisible(CXCompoundType type, String name, HashMap<CompoundDeclarationKey, TypeTrackerEntry> publicEntries, HashMap<CompoundDeclarationKey, TypeTrackerEntry> internalEntries, HashMap<CompoundDeclarationKey, TypeTrackerEntry> privateEntries, ParameterTypeList params) {
        ...
        if(type instanceof AbstractCXClassType) {
            AbstractCXClassType cxClassType = (AbstractCXClassType) type;
            for (AbstractCXClassType inherit : cxClassType.getReverseInheritanceOrder()) {
                if(params == null) {
    ...
    public void addBasicCompoundType(CXCompoundType type) {
        if(type instanceof AbstractCXClassType) return;
    ...
    public void removeParentlessStructFields() {
        ...
        for (CompoundDeclarationKey compoundDeclarationKey : publicFieldEntries.keySet()) {
            if(!(compoundDeclarationKey.type instanceof AbstractCXClassType) && !trackingTypes.contains(compoundDeclarationKey.type)) {
    ...
    public void addInternalField(AbstractCXClassType parent, String name, CXType type) {
        addCompoundTypeField(parent, name, type, internalFieldEntries, internalMethodEntries);
    }
    
    public void addPrivateField(AbstractCXClassType parent, String name, CXType type) {
    ...
    public void addInternalMethod(AbstractCXClassType parent, String name, CXType type, ParameterTypeList typeList) {
        addCompoundTypeMethodEntry(parent, name, type, typeList, internalMethodEntries);
    }
    
    public void addPrivateMethod(AbstractCXClassType parent, String name, CXType type, ParameterTypeList typeList) {
        addCompoundTypeMethodEntry(parent, name, type, typeList, privateMethodEntries);
    }
    
    public void addConstructor(Visibility visibility, AbstractCXClassType owner, ParameterTypeList parameterTypeList) {
    ...
    public boolean constructorVisible(AbstractCXClassType owner, ParameterTypeList parameterTypeList) {
        ConstructorKey constructorKey = new ConstructorKey(owner, parameterTypeList);
    ...
    public void addCompoundTypeMethodEntry(CXCompoundType parent, String name, CXType type, ParameterTypeList typeList,
        ...
        if(methodVisible(parent, name, typeList)) {
            assert parent instanceof AbstractCXClassType;
            // System.out.println(((AbstractCXClassType) parent).classInfo());
    ...
    public CXType getFieldType(AbstractCXClassType owner, String name) {
        for (AbstractCXClassType cxClassType : owner.getLineage()) {
    ...
    public CXType getMethodType(AbstractCXClassType owner, String name, ParameterTypeList typeList) {
        for (AbstractCXClassType cxClassType : owner.getReverseInheritanceOrder()) {
    ...
    public static VariableTypeTracker getTracker(AbstractCXClassType cxClassType) {
        return classTrackers.get(cxClassType);
    }
    
    public static boolean trackerPresent(AbstractCXClassType cxClassType) {
